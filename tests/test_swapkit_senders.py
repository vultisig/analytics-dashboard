...
class TestBackfill(unittest.TestCase):
    def test_updates_other_and_kyberswap_only(self):
        ...
        self.assertEqual(set(params[1]), set(all_swapkit_senders()))
        self.assertEqual(set(params[2]), set(BACKFILL_SOURCE_PROTOCOLS))
        self.assertNotIn("1inch", params[2])


class TestPlanEnrichment(unittest.TestCase):
    def test_skwrap_estimates_volume(self):
        updates, stamp = plan_swapkit_enrichment(SKWRAP, 20.0, 0)
        self.assertEqual(updates["actual_fee_usd"], 20.0)
        self.assertEqual(updates["swap_volume_usd"], 4000.0)
        self.assertEqual(stamp, "estimated")
    ...
    def test_payout_prices_fee_without_volume(self):
        ...
        self.assertNotIn("swap_volume_usd", updates)
    ...
    def test_existing_fee_not_rewritten(self):
        ...
        self.assertNotIn("actual_fee_usd", updates)
        self.assertEqual(updates["swap_volume_usd"], 4000.0)
        self.assertEqual(stamp, "estimated")
