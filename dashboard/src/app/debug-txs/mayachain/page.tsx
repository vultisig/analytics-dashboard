...
export default function MayachainDebugPage() {
    ...
    const fetchData = async () => {
        try {
            const res = await fetch('/api/debug/txs?chain=mayachain');
    ...
    const formatAmount = (amount: string | number, decimals: number = 10) => {
        if (!amount) return '0';
    ...
    return (
        <div className="p-8 min-h-screen bg-[#0A0F1E] text-slate-300">
            <h1 className="text-3xl font-bold text-white mb-6">MAYAChain Transactions Debug</h1>
                        ...
                        {transactions.map((tx) => (
                            <tr key={tx.tx_hash} className="border-b border-slate-800 hover:bg-[#0F1629]">
                                ...
                                <td className="px-4 py-3 font-mono">
                                    <a
                                        href={`https://www.mayascan.org/tx/${tx.tx_hash}`}
                                        target="_blank"
                                        rel="noreferrer"
